...
cParticle_Fire::cParticle_Fire(D3DXVECTOR3 *Target)
{
	...
	Seting_Smoke_Ver();
	pos_xyz = Target;
	m_Light.p = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	m_Light.m_Alpha = 50.f;
	m_Light.c = D3DCOLOR_ARGB((int)m_Light.m_Alpha, 255, 255, 255);
}
...
void cParticle_Fire::Seting_Fire_Ver()
{
	...
	// 포인트 스케일링 Factor값 설정
	g_pD3DDevice->SetRenderState(D3DRS_POINTSCALE_A, FtoDw(0.1f));
	g_pD3DDevice->SetRenderState(D3DRS_POINTSCALE_B, FtoDw(0.1f));
	g_pD3DDevice->SetRenderState(D3DRS_POINTSCALE_C, FtoDw(0.1f));
...
void cParticle_Fire::Seting_Fire_Ver_2()
{
	for (int i = 0; i < m_Particle_Vec.size(); ++i)
	{
		//Particle_Data Save = new Particle_Data;
...
void cParticle_Fire::Seting_Smoke_Ver_2()
{
	for (int i = 0; i < m_Particle_Smoke_Vec.size(); ++i)
	{
		//Particle_Data Save = new Particle_Data;
...
void cParticle_Fire::Render()
{
	...
	// 포인트 사이즈 설정
	g_pD3DDevice->SetRenderState(D3DRS_POINTSIZE, FtoDw(0.00001f));
	...
	// 포인트 최소 크기
	g_pD3DDevice->SetRenderState(D3DRS_POINTSIZE_MIN, FtoDw(0.0000000000001f));
	...
	g_pD3DDevice->SetRenderState(D3DRS_POINTSCALEENABLE, true);
	g_pD3DDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
	g_pD3DDevice->SetRenderState(D3DRS_POINTSIZE, FtoDw(1000000));
	g_pD3DDevice->SetRenderState(D3DRS_POINTSIZE_MIN, FtoDw(0.0f));
	g_pD3DDevice->SetRenderState(D3DRS_POINTSIZE_MAX, FtoDw(1000000));
	g_pD3DDevice->SetTexture(0, g_pTextureManager->GetTexture("Test/Light.png"));
	g_pD3DDevice->DrawPrimitiveUP(D3DPT_POINTLIST,
		1,
		&m_Light,
		sizeof(Particle_Data));
...
void cParticle_Fire::Fire_Start_OnOff()
{
